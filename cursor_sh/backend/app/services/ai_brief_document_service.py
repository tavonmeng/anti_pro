"""Extract Brief information from documents uploaded in an AI chat."""
...
MAX_BRIEF_DOCUMENT_ATTACHMENTS = 3
MAX_BRIEF_DOCUMENT_BYTES = 50 * 1024 * 1024
...
def is_brief_document_attachment(attachment) -> bool:
    """Return True for documents handled or explicitly rejected by this pipeline."""
    extension = _attachment_extension(attachment)
    return extension in SUPPORTED_BRIEF_DOCUMENT_EXTENSIONS
...
async def extract_uploaded_brief_documents(
    ...
    *,
    ...
    documents = [
        ...
        if is_brief_document_attachment(item)
    ]
    if not documents:
        return BriefDocumentExtraction()

    result = BriefDocumentExtraction()
    for attachment in documents[:MAX_BRIEF_DOCUMENT_ATTACHMENTS]:
        ...
        try:
            ...
            if os.path.getsize(parse_path) > MAX_BRIEF_DOCUMENT_BYTES:
                raise ValueError("文档文件过大，暂不处理")
            parse_filename = (
                filename
                if Path(filename).suffix.lower() in SUPPORTED_BRIEF_DOCUMENT_EXTENSIONS
                else f"{filename}{extension}"
            )
            sections = parse_document(parse_path, parse_filename)
            ...
            if not any(chunk.strip() for chunk in chunks):
                raise ValueError("文档没有可提取的文本，可能是扫描件或图片版文件")
            ...
            log_business_event(
                logger,
                "ai_brief_document_extract_failed",
                level="warning",
                user_id=user_id,
                filename=filename,
...
def build_brief_document_confirmation_reply(result: BriefDocumentExtraction) -> str:
    """Create the user-visible review message for an extracted document Brief."""
...
def build_brief_document_revision_reply(updates: dict[str, str]) -> str:
    """Acknowledge only the document Brief fields explicitly corrected by the user."""
...
async def _extract_brief_updates(chunks: list[str], filename: str) -> dict[str, str]:
    ...
    for index, chunk in enumerate(chunks, start=1):
        ...
        try:
            ...
            log_business_event(
                logger,
                "ai_brief_document_chunk_extract_failed",
                level="warning",
                filename=filename,
                chunk_index=index,
...
def _brief_extract_prompt() -> str:
    ...
    return (
        ...
        "只抽取文档中明确写出的信息，不推测、不补全、不把公司资料或案例当成当前项目需求。"
...
def _attachment_name(attachment) -> str:
    name = str(getattr(attachment, "name", "") or "上传的文档")
    return os.path.basename(name) or "上传的文档"


def _attachment_extension(attachment) -> str:
    name = str(getattr(attachment, "name", "") or getattr(attachment, "url", ""))
    extension = Path(name.split("?", 1)[0]).suffix.lower()
    if extension:
        return extension
    mime = str(getattr(attachment, "type", "") or "").lower()
    return BRIEF_DOCUMENT_MIME_EXTENSIONS.get(mime, "")
...
def _materialize_attachment(attachment, *, user_id: str) -> tuple[str, str]:
    ...
    if settings.OSS_ENABLED:
        ...
        extension = _attachment_extension(attachment)
        suffix = extension if extension in SUPPORTED_BRIEF_DOCUMENT_EXTENSIONS else ".bin"
        fd, temp_path = tempfile.mkstemp(prefix="ai_brief_document_", suffix=suffix)
        os.close(fd)
...
def _public_failure_message(exc: Exception) -> str:
    message = str(exc)
    if isinstance(exc, ValueError) and ("扫描件" in message or "图片版" in message):
        return "没有可提取的文字，可能是扫描件或图片版文件"
    ...
    if isinstance(exc, DocumentParseError):
        if "Word" in message or "antiword" in message:
            return "Word 解析失败"
        if "PDF" in message:
            return "PDF 解析失败"
        return "文档解析失败"
