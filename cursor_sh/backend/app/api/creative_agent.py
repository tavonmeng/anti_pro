...
async def start_auto_run(
    ...
    return {"code": 200, "data": run, "message": "Hermes 创意 Agent 已启动"}
...
async def continue_with_feedback(
    ...
    if request.wait_for_completion:
        ...
        background_tasks.add_task(creative_agent_service.watch_hermes_run, run["id"])
    return {"code": 200, "data": run, "message": "Hermes 已根据设计师反馈继续迭代"}
...
async def evaluate_idea(
    ...
    if request.wait_for_completion:
        ...
        background_tasks.add_task(creative_agent_service.watch_hermes_run, run["id"])
    return {"code": 200, "data": run, "message": "Hermes 创意质检已启动"}
...
async def iterate_idea(
    ...
    if request.wait_for_completion:
        ...
        background_tasks.add_task(creative_agent_service.watch_hermes_run, run["id"])
    return {"code": 200, "data": run, "message": "Hermes 创意迭代已启动"}
...
async def stream_run_events(
    ...
    return StreamingResponse(
        creative_agent_service.stream_run_events(run_id),
